/**
 * Markdown to HTML API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the Markdown to HTML API.
 ...
 */
async function callMarkdowntoHTMLAPIWithErrorHandling(requestData) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callMarkdowntoHTMLAPIWithErrorHandling({
